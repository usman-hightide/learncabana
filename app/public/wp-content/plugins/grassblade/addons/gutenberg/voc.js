...
const visibleOnCompletionControls = wp.compose.createHigherOrderComponent(BlockEdit => {
  return props => {
    ...
    function get_xapi_contents_on_page() {
      ...
      console.log(xapi_content_ids);
      gb_block_data.post_content.forEach(function (post) {
        if (typeof xapi_content_ids[post.id] != "undefined") {
          var item = {
            value: post.id,
    ...
    function gb_voc_selected_content() {
      var c = typeof props.attributes.className == "string" ? props.attributes.className.split(" ").filter(function (v) {
        ...
        if (check == null) // && (typeof xapi_content_ids[check[1]] == "undefined" || content_id != check[1]) )
          return false;
        return v != "";
      }) : [];
    ...
    function gb_voc_content_selected(content_id) {
      var c = typeof props.attributes.className == "string" ? props.attributes.className.split(" ").filter(function (v) {
        var check = v.match(/^gb_voc_(\d*)/);
        if (v == "gb_voc" || check != null) // && (typeof xapi_content_ids[check[1]] == "undefined" || content_id != check[1]) )
          return false;
        return v != "";
      }) : [];
    ...
    if (!featureDisabled && visibleOnCompletionEnabled && selectedOption) {
      jQuery("#block-" + props.clientId).addClass("gb_voc"); //jQuery("#block-" + props.clientId).data("gb_voc_msg", "(This block is visible only when " + selectedOption + " is completed.)");
