...
class Notice implements \IWPML_Action, \IWPML_Backend_Action {
	...
	/**
	 * The auto-migration consolidated notice owns the sitekey form whenever it's on screen —
	 * either because we have migration data (success/independent flow) or because a migration
	 * attempt failed (failure flow may render even with no MIGRATION_DATA captured yet).
	 * Outside that flow (manual unregister, etc.) this notice resumes its normal role.
	 */
