...
class Database {
	...
	static private function upgrade_table_snc_file_info() {
		...
		/**
		 * Update file_name and URL column types
		 *
		 *  @since 4.3.0.3
		 */
		$file_name_type_check = self::check_column_type( self::$tbl_modules, 'file_name', 'text' );
		if ( empty( $file_name_type_check ) ) {
			$query = sprintf( "ALTER TABLE %s MODIFY COLUMN `file_name` text", self::$tbl_modules );
			$wpdb->query( $query );
			if ( $wpdb->last_error ){
				$result = false;
			} else {
				$query = sprintf( "ALTER TABLE %s MODIFY COLUMN `url` longtext", self::$tbl_modules );
				$wpdb->query( $query );
				if ( $wpdb->last_error ){
					$result = false;
				}
	...
	/**
	 * Checks that database table column matches the type.
	 *
	 * @param string $table_name    Database table name.
	 * @param string $col_name      Table column name.
	 * @param string $col_type      Table column type.
	 */
	static public function check_column_type( $table_name, $col_name, $col_type ) {
		global $wpdb;
		$diffs = 0;
