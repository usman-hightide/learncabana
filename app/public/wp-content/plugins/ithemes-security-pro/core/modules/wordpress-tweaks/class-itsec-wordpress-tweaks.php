...
final class ITSEC_WordPress_Tweaks {
	...
	public function filter_rest_dispatch_request( $result, $request, $route_regex, $handler ) {
		...
		$error = new WP_Error( 'itsec_rest_api_access_restricted', __( 'You do not have sufficient permission to access this endpoint. Access to REST API requests is restricted by Kadence Security settings.', 'it-l10n-ithemes-security-pro' ), array(
			'status' => $code,
		) );
