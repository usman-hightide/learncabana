...
class ITSEC_Brute_Force {
	...
	public function itsec_lockout_modules( $lockout_modules, $translate ) {
		...
		$lockout_modules['brute_force'] = array(
			'type'   => 'brute_force',
			'reason' => $translate ? __( 'Too many bad login attempts', 'it-l10n-ithemes-security-pro' ) : 'Too many bad login attempts',
			'label'  => $translate ? __( 'Brute Force', 'it-l10n-ithemes-security-pro' ) : 'Brute Force',
		...
		$lockout_modules['brute_force_admin_user'] = array(
			...
			'reason' => $translate ? __( 'User tried to login as "admin."', 'it-l10n-ithemes-security-pro' ) : 'User tried to login as "admin."',
			'label'  => $translate ? __( 'Brute Force "admin" Username', 'it-l10n-ithemes-security-pro' ) : 'Brute Force "admin" Username',
			'host'   => 1,
			'user'   => 1,
			'period' => $this->settings['check_period']
		);
