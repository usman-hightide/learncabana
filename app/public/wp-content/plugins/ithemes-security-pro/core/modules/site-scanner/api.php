...
class ITSEC_Site_Scanner_API {
	...
	private static function generate_signature( $json ) {
		if ( ! ITSEC_Core::is_pro() ) {
			return new WP_Error( 'non_active_license', __( 'Not a Kadence Security Pro install.', 'it-l10n-ithemes-security-pro' ) );
		}

		if ( ! isset( $GLOBALS['ithemes_updater_path'] ) ) {
			return new WP_Error( 'updater_not_available', __( 'Could not find the Kadence updater.', 'it-l10n-ithemes-security-pro' ) );
		}
		...
		if ( empty( $keys['ithemes-security-pro'] ) ) {
			return new WP_Error( 'non_active_license', __( 'Kadence Security Pro is not activated.', 'it-l10n-ithemes-security-pro' ) );
		}
