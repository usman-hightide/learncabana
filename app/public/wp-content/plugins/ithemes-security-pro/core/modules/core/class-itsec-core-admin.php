...
class ITSEC_Core_Admin implements Runnable {
	...
	public function add_ip_detection_notice() {
		...
		$text = esc_html__( 'Important: Some Kadence Security features are disabled because IP Detection has not been configured.', 'it-l10n-ithemes-security-pro' );
		...
		printf(
			...
			$img,
			__( 'Kadence Security Logo', 'it-l10n-ithemes-security-pro' ),
			$text
		);
	}
	...
	public function customize_optin( $args, $slug ) {
		if ( $slug === 'solid-security' ) {
			...
			$args['plugin_logo_alt']    = __( 'Kadence Security Logo', 'it-l10n-ithemes-security-pro' );
			...
			$args['heading']            = __( 'Help us improve Kadence Security.', 'it-l10n-ithemes-security-pro' );
	...
	public function add_site_health_info( $info ) {
		...
		$info['solid-security'] = [
			'label'  => __( 'Kadence Security', 'it-l10n-ithemes-security-pro' ),
			'fields' => [
				'pro'           => [
