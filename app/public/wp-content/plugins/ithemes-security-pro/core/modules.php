...
final class ITSEC_Modules implements Import_Export_Source {
	...
	public static function register_module( $slug, $path ) {
		...
		if ( ! is_dir( $path ) ) {
			trigger_error( sprintf( __( 'An attempt to register the %1$s module failed since the supplied path (%2$s) is invalid. This could indicate an invalid modification or incomplete installation of the Kadence Security plugin. Please reinstall the plugin and try again.', 'it-l10n-ithemes-security-pro' ), $slug, $path ) );
	...
	public static function initialize_container() {
		...
		if ( is_callable( $load ) && ! self::get_instance()->initialized_container ) {
			...
			self::get_instance()->container->get( Harbor_Provider::class )->run();
		}
