...
class ITSEC_Lib_Config_File {
	...
	public static function get_minimal_server_config() {
		...
		$comment_delimiter = self::get_comment_delimiter( $server );
		$modification = "$comment_delimiter " . __( 'Kadence Security preserved the following settings as removing them could prevent the site from functioning correctly.', 'it-l10n-ithemes-security-pro' ) . "\n$modification";
...
//			return new WP_Error( 'itsec-lib-config-file-server-config-file-updates-disabled', __( 'Updates to the server config file are disabled via a filter.', 'it-l10n-ithemes-security-pro' ) );
	...
	protected static function get_prepared_modification( $modification, $comment_delimiter ) {
		...
		$modification  = "$comment_delimiter BEGIN Kadence Security - " . __( 'Do not modify or remove this line', 'it-l10n-ithemes-security-pro' ) . "\n";
		$modification .= "$comment_delimiter Kadence Security Config Details: " . self::FORMAT_VERSION . "\n";
		$modification .= "$supplied_modification\n";
		$modification .= "$comment_delimiter END Kadence Security - " . __( 'Do not modify or remove this line', 'it-l10n-ithemes-security-pro' );
