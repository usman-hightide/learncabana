...
class ITSEC_Lib_Distributed_Storage {
		...
		foreach ( $config as $key => $value ) {
			...
			if ( array_key_exists( 'serialize', $value ) || array_key_exists( 'unserialize', $value ) ) {
				if ( ! isset( $value['serialize'] ) ) {
					_doing_it_wrong( __CLASS__, 'Kadence Security: Serialize function required when using unserialize.', '4.5.0' );
				} elseif ( ! is_callable( $value['serialize'] ) ) {
					_doing_it_wrong( __CLASS__, 'Kadence Security: Serialize function must be callable.', '4.5.0' );
				} else {
					$valid = true;
				}

				if ( ! isset( $value['unserialize'] ) ) {
					_doing_it_wrong( __CLASS__, 'Kadence Security: Unserialize function required when using serialize.', '4.5.0' );
				} elseif ( ! is_callable( $value['unserialize'] ) ) {
					_doing_it_wrong( __CLASS__, 'Kadence Security: Unserialize function must be callable.', '4.5.0' );
				} else {
					$valid = true;
				}
			} else {
	...
	public function get( $key ) {
		...
		if ( ! isset( $this->config[ $key ] ) ) {
			_doing_it_wrong( __METHOD__, "Kadence Security: Unsupported key '{$key}' for '{$this->name}' storage.", '4.5.0' );
	...
	public function append( $key, $value ) {
		...
		if ( ! $config['chunk'] ) {
			_doing_it_wrong( __METHOD__, "Kadence Security: Cannot append to non-chunked key '{$key}' for '{$this->name}' storage.", '4.5.0' );
	...
	public function set_from_iterator( $key, $iterator ) {
		...
		if ( ! $config['chunk'] ) {
			_doing_it_wrong( __METHOD__, "Kadence Security: Cannot set from iterator to non-chunked key '{$key}' for '{$this->name}' storage.", '4.5.0' );
	...
	private function load_split_option( $key, $chunk = 0 ) {
		...
		if ( ! is_array( $option ) ) {
			trigger_error( "Kadence Security: Non-array value encountered for chunked key '{$key}' in storage '{$this->name}'." );
