	...
	public function discover( WP_REST_Request $request ) {
		...
		if ( $status !== 200 ) {
			return new WP_Error(
				...
				sprintf( __( 'Kadence Security REST API index returned a non-200 status code (%d).', 'it-l10n-ithemes-security-pro' ), $status ),
				[ 'status' => WP_Http::BAD_REQUEST ]
			);
		}
		...
		if ( ! $body || ! $itsec_data = json_decode( $body, true ) ) {
			return new WP_Error(
				...
				__( 'Kadence Security REST API index returned no data.', 'it-l10n-ithemes-security-pro' ),
				[ 'status' => WP_HTTP::BAD_REQUEST ]
			);
		}
