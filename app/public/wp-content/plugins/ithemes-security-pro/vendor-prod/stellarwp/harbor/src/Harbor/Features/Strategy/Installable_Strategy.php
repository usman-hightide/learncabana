...
namespace iThemesSecurity\Strauss\LiquidWeb\Harbor\Features\Strategy;
	...
	final public function update() {
		...
		if ( ! $this->check_active() ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( '"%s" is not installed or active. Enable it first before updating.', 'it-l10n-ithemes-security-pro' ),
		...
		if ( ! $this->check_update_available() ) {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: feature name */
					__( 'No update is available for "%s".', 'it-l10n-ithemes-security-pro' ),
					$this->feature->get_name()
				)
			);
		}
		...
		if ( ! $this->acquire_lock( self::LOCK_KEY ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: feature name */
					__( 'Another installable feature is already being installed. Cannot update "%s" right now. Please try again in a few moments.', 'it-l10n-ithemes-security-pro' ),
					$this->feature->get_name()
				)
			);
		}
	...
	final protected function ensure_installed() {
		...
		if ( ! $this->acquire_lock( self::LOCK_KEY ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: feature name */
					__( 'Another installable feature is already being installed. Cannot install "%s" right now. Please try again in a few moments.', 'it-l10n-ithemes-security-pro' ),
					$this->feature->get_name()
				)
			);
		}
		...
		try {
			...
			if ( ! $this->check_installed() ) {
				...
				return new WP_Error(
					...
					sprintf(
						/* translators: %s: feature name */
						__( 'The extension was not found after installing "%s". The downloaded package may have an unexpected directory structure.', 'it-l10n-ithemes-security-pro' ),
						$this->feature->get_name()
					)
				);
			}
	...
	protected function run_upgrader( callable $operation, WP_Ajax_Upgrader_Skin $skin, string $error_code, bool $is_update ) {
		...
		try {
			...
			return new WP_Error(
				...
				$is_update
					? sprintf(
						/* translators: %s: feature name */
						__( 'A fatal error occurred while updating "%s".', 'it-l10n-ithemes-security-pro' ),
						$name
					)
					: sprintf(
						/* translators: %s: feature name */
						__( 'A fatal error occurred while installing "%s".', 'it-l10n-ithemes-security-pro' ),
						$name
					)
			);
		}
			...
			return new WP_Error(
				...
				$is_update
					? sprintf(
						...
						__( 'Failed updating "%1$s": %2$s', 'it-l10n-ithemes-security-pro' ),
						$name,
						$result->get_error_message()
					)
					: sprintf(
						/* translators: %1$s: feature name, %2$s: error message */
						__( 'Failed installing "%1$s": %2$s', 'it-l10n-ithemes-security-pro' ),
						$name,
						$result->get_error_message()
					)
			);
		...
		if ( $result !== true ) {
			...
			$message = $skin_errors->has_errors()
				...
				: ( $is_update
					? __( 'An unknown error occurred while updating.', 'it-l10n-ithemes-security-pro' )
					: __( 'An unknown error occurred while installing.', 'it-l10n-ithemes-security-pro' )
				);
