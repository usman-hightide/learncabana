...
final class ITSEC_Import_Export_Importer {
	 ...
	 * @static
	 *
	 * @param string $file File path for the file to pull Kadence Security settings from.
	 *
			...
			if ( true === $unzip_result ) {
				...
				foreach ( $files as $file ) {
					...
					if ( is_wp_error( $result ) ) {
						...
						return new WP_Error( 'multiple_settings_files_found', __( 'The supplied ZIP file contained more than one JSON file with valid Kadence Security settings. Only ZIP files with one JSON file of valid settings are permitted. Please ensure that a valid export file is supplied.', 'it-l10n-ithemes-security-pro' ) );
					} else {
						$settings = $result;
					}
				}
				...
				if ( isset( $settings ) ) {
					...
					return new WP_Error( 'valid_json_settings_file_not_found', __( 'The supplied ZIP file did not contain a JSON file with valid Kadence Security settings. Please ensure that a valid export file is supplied.', 'it-l10n-ithemes-security-pro' ) );
				}
			}
		}
	...
	/**
	 * Returns validated Kadence Security settings the supplied JSON file.
	 *
	 * @static
	 *
	 * @param string $file File path to the JSON file to pull the settings from.
	 *
	 * @return array|WP_Error Returns an array of valid Kadence Security settings, or a WP_Error object otherwise.
	 */
	private static function get_data_from_json_file( $file ) {
		...
		foreach ( $data['options'] as $index => $option ) {
			...
			if ( 'itsec' !== substr( $option['name'], 0, 5 ) ) {
				return new WP_Error( 'non_security_settings_found', __( 'The settings file contains settings that are not for Kadence Security. These settings will not be imported. Please supply an export file from Kadence Security.', 'it-l10n-ithemes-security-pro' ) );
			}
