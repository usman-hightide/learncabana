...
class PhpRedisConnection extends Connection implements ConnectionInterface
{
    ...
    public function listKeys(?string $pattern = null, int $limit = 500): Generator
    {
        ...
        do {
            $keys = $this->client->scan($iterator, $pattern, $limit);
    ...
    public function commands(Transaction $tx)
    {
        ...
        if ($debug) {
            ...
            $this->log->debug("Executed transaction in {$ms}ms", $context + [
                'time' => $wait,
                'memory' => $memory,
                'result' => $results,
            ]);
    ...
    /**
     * The `CONFIG` container command for runtime configuration commands.
     *
    ...
    public function config($command, ...$args)
    {
        ...
        if ($command === 'resetstat') {
            return version_compare((string) phpversion('redis'), '6.0', '<')
                ? $this->rawCommand('config', $command)
                : $this->command('config', [$command]);
        }

        array_unshift($args, $command);

        return $this->command('config', $args);
    }

    /**
     * The `SLOWLOG` container command for slowlog commands.
     *
     * @param  string  $command
     * @param  string  ...$args
     * @return mixed
    ...
    public function slowlog($command, ...$args)
    {
        ...
        if ($command == 'reset') {
            return version_compare((string) phpversion('redis'), '6.0', '<')
                ? $this->rawCommand('slowlog', $command)
                : $this->command('slowlog', [$command]);
        }
