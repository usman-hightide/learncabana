...
class LogWatcher extends Notify
{
    ...
    public function display($finish = false)
    {
        ...
        foreach ($this->measurements as $measurement) {
            ...
            if ($measurement->hostname) {
                $message[] = sprintf("\e[32m%s %s:\e[0m", $measurement->rfc3339(), $measurement->hostname);
            } else {
                $message[] = sprintf("\e[32m%s:\e[0m", $measurement->rfc3339());
            }

            if ($measurement->path) {
                $message[] = sprintf("\e[2mpath=\e[0m\e[32m\"%s\"\e[0m", urldecode($measurement->path));
            }

            $message[] = $this->formatMeasurement(
                $measurement->toArray(),
                $metrics
            );

            Streams::line(implode(' ', $message));
        }
