...
trait Health
{
    ...
    protected function healthTestTrackingTableMaxKeys(Diagnostics $diagnostics)
    {
        ...
        $maxKeys = $diagnostics->trackingTableMaxKeys();

        if ($maxKeys === false) {
            return [
                'label' => 'Redis tracking keys check failed',
                'description' => '<p>Unable to retrieve the <code>tracking-table-max-keys</code> configuration from the Redis server.</p>',
                'badge' => ['label' => 'Object Cache Pro', 'color' => 'orange'],
                'status' => 'recommended',
