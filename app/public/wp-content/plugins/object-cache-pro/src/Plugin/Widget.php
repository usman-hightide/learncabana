...
trait Widget
{
    ...
    public function handleWidgetActions()
    {
        ...
        switch ($action) {
            ...
            case 'flush-network-cache':
                ...
                break;
            case 'flush-relay':
                $status = $this->flushRelay() ? 'relay-flushed' : 'relay-not-flushed';
                break;
            case 'reset-relay-ratios':
                $status = $this->resetRelayRatios() ? 'relay-ratios-reset' : 'relay-ratios-not-reset';
                break;
            case 'enable-dropin':
    ...
    public function displayWidgetNotice()
    {
        ...
        switch ($status) {
            ...
            case 'network-cache-not-flushed':
                ...
                break;
            case 'relay-flushed':
                echo $notice('success', 'The Relay memory was flushed.');
                break;
            case 'relay-not-flushed':
                echo $notice('error', 'The Relay memory could not be flushed.');
                break;
            case 'relay-ratios-reset':
                echo $notice('success', 'The adaptive cache ratios have been reset.');
                break;
            case 'relay-ratios-not-reset':
                echo $notice('error', 'The adaptive cache rations could not be reset.');
                break;
            case 'dropin-enabled':
    ...
    /**
     * Updates the flush timestamp in the cache metadata.
     *
     * @return bool
     */
    protected function flushRelay()
    {
        global $wp_object_cache;

        if (! $this->diagnostics()->usingRelay()) {
            return false;
        }

        $wp_object_cache->metadata('relay.flushed_at', microtime(true));
        $wp_object_cache->writeMetadata();

        return true;
    }

    /**
     * Updates the ratio reset timestamp in the cache metadata.
     *
     * @return bool
     */
    protected function resetRelayRatios()
    {
        global $wp_object_cache;
        ...
        $wp_object_cache->metadata('relay.ratios_reset_at', microtime(true));
        $wp_object_cache->writeMetadata();
