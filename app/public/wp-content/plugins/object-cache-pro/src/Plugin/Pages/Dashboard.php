...
class Dashboard extends Page
{
    ...
    protected function addRelayWidget()
    {
        if (extension_loaded('relay')) {
            add_meta_box(
                'objectcache_relay_overview',
                'Relay',
                [$this, 'renderRelayWidget'],
                $this->plugin->screenId(),
                'normal'
            );
        ...
        //     },
        //     $this->plugin->screenId(),
        //     $this->plugin->analyticsEnabled() ? 'column4' : 'side'
        // );
    }

    /**
     * Render the Relay overview widget.
     *
     * @return void
     */
    public function renderRelayWidget()
    {
        require __DIR__ . '/../templates/widgets/relay.phtml';
    }
    ...
    protected function filterMetrics(array $metrics)
    {
        ...
        if ($this->plugin->diagnostics()->usingRelayCache()) {
            ...
            if ($this->plugin->diagnostics()->usingRelayAdaptiveCache()) {
                array_push($order, 'relay-adaptive-loadfactor');
            }
        }
