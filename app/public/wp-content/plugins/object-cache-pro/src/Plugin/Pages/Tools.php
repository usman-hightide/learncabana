...
class Tools extends Page
{
    ...
    public function boot()
    {
        ...
        $this->addHealthWidget();
        $this->addGroupsWidget();

        $this->addSlowLogWidget();
        $this->addCommandsWidget();

        if ($this->plugin->diagnostics()->usingRelayAdaptiveCache()) {
            $this->addAdaptiveCacheWidget();
        }
    ...
    /**
     * Adds the "Diagnostics" widget.
     *
     * @return void
     */
    protected function addHealthWidget()
    {
        add_meta_box(
            'objectcache_health',
            'Health',
            function () {
                require __DIR__ . '/../templates/widgets/tools/health.phtml';
            },
            $this->plugin->screenId(),
            'side'
        );
    ...
    /**
     * Adds the "Slow Log" widget.
     *
     * @return void
     */
    protected function addSlowLogWidget()
    {
        add_meta_box(
            'objectcache_slowlog',
            'Slow Commands',
            function () {
                require __DIR__ . '/../templates/widgets/tools/slowlog.phtml';
            },
            $this->plugin->screenId(),
            'column3'
        );
    ...
    /**
     * Adds the "Command Stats" widget.
     *
     * @return void
     */
    protected function addCommandsWidget()
    {
        add_meta_box(
            'objectcache_commandstats',
            'Command Statistics',
            function () {
                require __DIR__ . '/../templates/widgets/tools/commands.phtml';
            },
            $this->plugin->screenId(),
            'column3'
        );
    ...
    /**
     * Adds the adaptive cache widget.
     *
     * @return void
     */
    protected function addAdaptiveCacheWidget()
    {
        add_meta_box(
            'objectcache_adaptive',
            'Relay Adaptive Cache',
            function () {
                require __DIR__ . '/../templates/widgets/tools/adaptive.phtml';
            },
            $this->plugin->screenId(),
            'column4'
        );
    ...
    protected function flushlogCaller(string $backtrace)
    {
         ...
         * @param  string  $backtrace  The cleaned, partial comma-separated string backtrace summary.
         */
        $caller = (string) apply_filters(
            'objectcache_flushlog_caller',
            strstr($backtrace, ',', true) ?: $backtrace,
            $backtrace
        );
        ...
        if (strpos($caller, 'wp_upgrade') !== false) {
            return 'WordPress upgrade';
        }
