...
class Analytics extends Controller
{
    ...
    public function get_items($request)
    {
        ...
        $range = array_slice(array_reverse(range($min, $max, $interval)), 0, 31, true);
        ...
        // Convert cumulative totals to per-interval deltas
        for ($i = 0, $limit = count($collection) - 1; $i < $limit; $i++) {
            foreach (['redis-hits', 'relay-hits', 'redis-misses', 'relay-misses'] as $metric) {
                if (! array_key_exists($metric, $collection[$i])) {
                    continue;
                }

                foreach ($collection[$i][$metric] as $key => $value) {
                    $collection[$i][$metric][$key] = $value - $collection[$i + 1][$metric][$key];
                }
            }
        }
