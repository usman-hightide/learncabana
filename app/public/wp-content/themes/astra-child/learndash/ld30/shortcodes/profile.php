		...
		<div class="ld-item-list ld-course-list">
			...
			<div class="ld-item-list-items" id="ld-main-course-list" data-ld-expand-list="true" data-ld-expand-id="<?php echo esc_attr( 'ld-main-course-list' ); ?>">
                ...
                $args = array(
                    ...
                    'meta_query'     => array(
                        array(
                            'key'     => 'learndash_course_order',
                            'compare' => 'EXISTS',
                        ),
                    ),
                    'orderby'        => 'meta_value_num', // Order by numeric meta value
                    'meta_key'       => 'learndash_course_order', // Specify the meta key to order by
                    'order'          => 'ASC', // Order ascending (you can change to 'DESC' for descending order)
                );
                ...
                if ( $course_query->have_posts() ) {
                    // Loop through the posts and store their IDs in the array
				...
				if ( ! empty( $user_courses ) ) :
					if ( ! empty( $post_ids ) ) :
						foreach ( $post_ids as $learndash_course_id ) :
							if( in_array( $learndash_course_id, $user_courses ) ) {
								learndash_get_template_part(
                    ...
                    foreach ( $user_courses as $learndash_course_id ) :
                        if( ! in_array( $learndash_course_id, $post_ids ) ) { 
                            learndash_get_template_part(
                                'shortcodes/profile/course-row.php',
                                array(
                                    'user_id'        => $user_id,
